...
Where everything lives once the pipeline has run:
...
| | |
|---|---|
| Episodes | 39 (ep01 – ep39, all present) |
| Pages | 289 |
| Base variants per page | 9 |
| Overlay variants per page | 4 (generated separately) |
| Renders | 289 cleaned + 288 initial* |

\* One page (`E39P08`) has a cleaned render but no matching initial render. The two variants sourced from the initial render (`jpeg`, `framed_jpeg`) have 288 files instead of 289 as a result — every other variant covers all 289 pages.
...
Generated by `synthesize_dataset.py`:
...
Generated by `synthesize_overlays.py` — composites synthetic Korean SFX text and speech bubbles on top of the base renders, so the model also learns to see past foreground text:
...
![All variants](assets/variants_grid.png)
...
# 4. Generate 9 base training variants (~80 min for all 289 pages)
...
### v1.7.4 · Progress bars + single-source assets
Added `tqdm` progress bars to `process_kra.py`, `synthesize_dataset.py`, and `synthesize_overlays.py`. Simplified `make_assets.py` back to sourcing the variant grid and demo GIF from a single episode (`ep01`) instead of spreading frames across episodes.

### v1.7.5 · Skip-existing + graceful tqdm fallback
`synthesize_dataset.py` and `synthesize_overlays.py` now skip pages whose output files already exist, so an interrupted run can resume without regenerating finished work. `tqdm` became optional — scripts fall back to plain print-based progress when it isn't installed.
...
### v1.7.7 · Overlay placement crash fix
`_place_edge` in `synthesize_overlays.py` could receive an invalid `randint` range when an overlay's width exceeded twice the panel width, raising an empty-range error mid-run. Fixed by clamping both bounds so the range is always valid.
...
### Why keep the noisy "initial" render instead of just the clean artwork?

`initial` is a training *input*, not a mistake to fix before using the dataset — it's the raw merged KRA export with whatever border and background the original page had. Pairing it with the clean `cleaned` target is what teaches the model what to remove.

### Do I need to regenerate everything if a run gets interrupted?

No. `process_kra.py`, `synthesize_dataset.py`, and `synthesize_overlays.py` all skip pages whose output files already exist, so an interrupted run can just be restarted with the same command (see `--retry-errors` in **How to run** for files that errored rather than were simply left unfinished).
